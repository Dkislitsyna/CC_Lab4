...
namespace Last
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            {
                c.SwaggerDoc("v1", new Microsoft.OpenApi.Models.OpenApiInfo { Title = "Last", Version = "v1" });
            });


            switch (Configuration["Storage:Type"].ToStorageEnum())
            {
                case StorageEnum.Avto:
                    services.AddSingleton<IStorage<Car>, Avto>();
                    break;
                case StorageEnum.FileStorage:
                    services.AddSingleton<IStorage<Car>>(
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
        {
            if (env.IsDevelopment())
            {
                ...
                app.UseSwagger();
                app.UseSwaggerUI(c => c.SwaggerEndpoint("/swagger/v1/swagger.json", "Last v1"));
            }
